...
def run_task_stream(task_id, env_client, llm_client, llm_model, backup_client=None, backup_model=None):
    """
    Streaming version of run_task() for UI.
    Yields step-by-step data instead of printing.
    """
    ...
    for step in range(1, 30):
        ...
        # Prepare input to LLM (same as your existing logic)
        user_msg = env_client.format_observation(obs)
        conversation.append({"role": "user", "content": user_msg})
        ...
        try:
            ...
            # fallback action
            assistant_text = '{"action_type":"query_logs","service":"api-gateway"}'

        conversation.append({"role": "assistant", "content": assistant_text})

        # Parse action (reuse your existing function)
        action = parse_action(assistant_text)
        ...
        logs = (
            ...
            else obs.get("last_action_result")
        )

        health = (
            obs.service_health
            if hasattr(obs, "service_health")
            else obs.get("service_health")
        )
